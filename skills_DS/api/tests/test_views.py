...
class TestViews(TestCase):
	...
	def test_answers_view(self):
		...
		data = self.client.post("/api/answers", {"age": 12, "gender": "male", "yearOfStudy": "2022"}, format='json').data
		self.user.refresh_from_db()
		self.assertIn("success", data)
		self.assertTrue(self.user.profile)
		self.assertEqual(self.user.profile.age, 12)
		data = self.client.post("/api/answers", {"age": 15, "gender": "male", "yearOfStudy": "2022"}, format='json').data
		self.user.refresh_from_db()
		self.assertIn("success", data)
		self.assertTrue(self.user.profile)
	...
	def get_jobs_view_test(self):
		...
		res = self.client.post("/api/get-jobs", {"position": "software developer", "location": "Kelowna BC", "country": "Canada", "remote": "allowed", "number": 3, "radius": 10}, format="json")
		self.assertEqual(res.status_code, 403)

		self.client.logout()
		self.client.login(email=self.user.email, password=self.user_password)
		data = self.client.post("/api/get-jobs", {"position": "software developer", "location": {"name": "kelowna british columbia", "lat": 49.887951899999997, "lng": -119.496010600000005}, "country": "CA", "remote": "allowed", "number": 3, "radius": 10}, format="json").data
		self.assertIn("success", data)
	...
	def test_list_skills_view(self):
		...
		Skill(name="test", location=location, job_title=job_title, count=5, verified=True).save()
		data = self.client.get("/api/list-skills", format="json").data
		self.assertEqual(len(data), 1)
		for x in range(21):
	...
	def test_update_skills_view(self):
		...
		for idx, skill in enumerate(skills):
			skill.save()
			skill_data.append({"job_title": "software developer", "skill": skill.name, "value": values[idx]})
